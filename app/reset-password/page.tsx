...
// 1. PRO MOVE: Volvemos a las variables de entorno seguras
...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    async function initializeSecureSession() {
      // Extraemos la llave sin importar por qué método la mande Supabase/Gmail
      ...
      if (sessionEstablished) {
         setHasSession(true)
         // 2. PRO MOVE: Limpiamos la URL para que no quede rastro de la llave secreta por seguridad
         window.history.replaceState({}, document.title, window.location.pathname)
      } else {
         setError("No se ha detectado una llave válida. Es posible que el enlace ya se haya usado.")
      }
