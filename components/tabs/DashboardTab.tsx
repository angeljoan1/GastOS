...
function HidableAmount({
  value, className = "", prefix = "", suffix = "€", decimals = 2, iconSize = "w-3.5 h-3.5",
}: {
  value: number; className?: string; prefix?: string
  suffix?: string; decimals?: number; iconSize?: string
}) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5">
      ...
      <button
        ...
        aria-pressed={!hidden}
        aria-label={hidden ? "Mostrar importe" : "Ocultar importe"}
...
export default function DashboardTab({
  categorias, cuentas, presupuestos,
}: {
      ...
      <div key="proyeccion_mes" className="bg-zinc-900 border border-zinc-800/70 rounded-2xl p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-zinc-800/60 rounded-xl p-3 space-y-1.5">
            ...
            <HidableAmount value={gastoDiario} className="text-lg font-light text-zinc-200" iconSize="w-3 h-3" />
          </div>
          <div className="bg-zinc-800/60 rounded-xl p-3 space-y-1.5">
            <p className="text-xs text-zinc-500">Gasto proyectado</p>
            <HidableAmount value={gastoProyectado} className="text-lg font-light text-red-400" iconSize="w-3 h-3" />
          </div>
        </div>
      ...
      <div key="media_diaria" className="bg-zinc-900 border border-zinc-800/70 rounded-2xl p-5">
        ...
        <div className="flex items-end justify-between gap-3">
          <div className="space-y-1">
            <HidableAmount value={gastoDiario} className="text-4xl font-light text-zinc-100" iconSize="w-4 h-4" />
          ...
          <div className="flex-1 space-y-2 min-w-0">
            ...
            <div className="space-y-1.5 pt-1">
              <div className="flex justify-between items-center text-xs gap-2">
                ...
                <HidableAmount value={totalIngresos} className="text-zinc-400" iconSize="w-3 h-3" />
              </div>
              <div className="flex justify-between items-center text-xs gap-2">
                <span className="text-zinc-600 flex-shrink-0">Gastos</span>
                <HidableAmount value={totalGastos} className="text-zinc-400" iconSize="w-3 h-3" />
              </div>
    ...
    <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-500">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <EncryptionBadge />
          {/* BUG #21 FIX: deshabilitado cuando ya estamos en el mes actual */}
          <button
            onClick={() => setSelectedDate(new Date(sy, sm + 1, 1))}
            disabled={esHoyMes}
